...
from img_loss import CrossEntropyLabelSmooth
...
def cal_acc(loader, netF, netB, netC, flag=False):
    ...
    mean_ent = torch.mean(img_loss.Entropy(all_output)).cpu().data.item()
...
def test_target(args):
    ...
    if args.da == 'oda':
        ...
        log_str = 'Training: {}, Task: {}, Accuracy = {:.2f}% / {:.2f}% / {:.2f}%'.format(args.trte, args.name, acc_os2, acc_os1, acc_unknown)
    else:
        if args.dset=='VISDA-C':
            acc, acc_list = cal_acc(dset_loaders['test'], netF, netB, netC, True)
            log_str = 'Training: {}, Task: {}, Accuracy = {:.2f}%'.format(args.trte, args.name, acc) + '\n' + acc_list
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='src_result')
